...
import { fetchOffer } from '../../../NativeModules/WebRTCNModule';
interface props{
  port:number;
  url:string;
}
const Client: React.FC<props> = ({port,url}) => {
  ...
  useEffect(() => {
    const initializeConnection = async () => {
      ...
      // Fetch the offer using the native module
      try {
        const offer = await fetchOffer();

        // Set remote description
        await peerConnectionRef.current.setRemoteDescription(new RTCSessionDescription(JSON.parse(offer)));
  ...
  const pingServer = async () => {
    try {
      const response = await fetch(`http://${url}:${port}/ping`);
